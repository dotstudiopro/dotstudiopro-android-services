...
public class SliderShowcaseSeriesVideoDetailsService_V1 implements CommonAsyncHttpClient_V1.ICommonAsyncHttpClient_V1 {
    ...
    public SliderShowcaseSeriesVideoDetailsService_V1(Context ctx, ArrayList<VideoInfoDTO> videoInfoDTOArrayListForSliderShowcase,
        ...
        this.numberOfVideosToFetchForSliderShowCase = numberOfVideosToFetchForSliderShowCase;
    ...
    public void processJSONResponseObjectForSliderShowcase(JSONObject response) {
        ...
        try {
            ...
            if (obj.getString("source").toLowerCase().equals("youtube")) {
                ...
                try {
                    if(obj.has("youtube")) {
                        videoInfoDTO.setYoutubeVideoID(obj.getJSONObject("youtube").getString("id"));
                    } else {
                        if(obj.has("video_m3u8")) {
                            ...
                            if(youtubeURL!=null && youtubeURL!="null" && youtubeURL.length()>4) {
                                ...
                                if(!youtubeVideoID.substring(0, 8).equals("http://"))
                                    youtubeVideoID = "http://"+youtubeVideoID;
            ...
            try {
                if(obj.has("custom_fields")) {
                    for(int i = 0; i < obj.getJSONArray("custom_fields").length(); i++) {
                        ...
                        JSONObject customFieldJSONObject = (JSONObject) obj.getJSONArray("custom_fields").get(i);
...

            {
                ...
                if(sourceValue.equals("Live")) {
                    ...
                    if(!(videoInfoDTO.isYoutubeVideo() && videoInfoDTO.getYoutubeVideoID() != null && videoInfoDTO.getYoutubeVideoID().length() > 0)) {
                        ...
                        try {
                            ...
                            /*if(!videoToPlayURL.substring(0, 8).equals("https://"))
                                videoToPlayURL = "https://"+videoToPlayURL;*/
                            ...
                            videoToPlayURL = serverURL + vodDir + videoFileName;
                            videoInfoDTO.setVideoToPlayURL(videoToPlayURL);
                ...
                if (midroll.equals("yes")) {
                    try {
                        ...
                        JSONArray linearOffset = obj.getJSONObject("ads").getJSONObject("offsets").getJSONArray("linear");
                        int[] linearOffsetInt = new int[linearOffset.length()];
                        int arrayLength = linearOffset.length();

                        int i = 0;
                        do {
                            linearOffsetInt[i] = Integer.parseInt("" + linearOffset.get(i));
                            i++;
                        } while (i < arrayLength);
        ...
        try {
            ...
            } catch (Exception e) { videoInfoDTO.setVideoTitle(""); }
            try {
                videoInfoDTO.setDescription(obj.getString("description"));
            } catch (Exception e) { videoInfoDTO.setDescription(""); }
        ...
        //videoInfoDTOFetchedSoFar++;
        videoInfoDTOArrayListForSliderShowcase.add(videoInfoDTO);

        try {
            if (indexOfThisVideo == (numberOfVideosToFetchForSliderShowCase-1)) {
                if(!someVideoDataMissing) {
                    for(int i = 0; i < videoInfoDTOArrayListForSliderShowcase.size(); i++) {
                        spotLightCategoriesDTOListForSliderShowcase.get(0).getVideoInfoDTOList().add(videoInfoDTOArrayListForSliderShowcase.get(i));
                    }
                    iSliderShowcaseSeriesVideoDetailsService_V1.callBackFromFetchSeriesVideoDetailsForSliderShowcase(videoInfoDTOArrayListForSliderShowcase);
                } else {
                    ApplicationConstants.PREVIOUS_LOADED_VIEW = ApplicationConstants.CATEGORIES_VIEW;
                    ApplicationConstants.CURRNET_LOADED_VIEW = ApplicationConstants.CHANNEL_VIEW;
                    iSliderShowcaseSeriesVideoDetailsService_V1.getSliderShowcaseSeriesVideoDetailsServiceError(ApplicationConstants.DATA_MISSING_SLIDER_SHOWCASE);
                    iSliderShowcaseSeriesVideoDetailsService_V1.callBackFromFetchSeriesVideoDetailsForSliderShowcase(videoInfoDTOArrayListForSliderShowcase);
                }
            } else {
                iSliderShowcaseSeriesVideoDetailsService_V1.fetchNextVideoFromPlaylistForSliderShowcase(videoInfoDTOArrayListForSliderShowcase.size());
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
