...
public class GetCategoriesFullDataService_V1 /*implements CommonAsyncHttpClient_V1.ICommonAsyncHttpClient_V1*/ {

    public IGetCategoriesFullDataService_V1 iGetCategoriesFullDataService_V1;
    public interface IGetCategoriesFullDataService_V1 {
        void getCategoriesFullDataServiceResponse(
    ...
    public void onResultHandler1(JSONObject response) {
        resultProcessingForGetCategoriesFullDataService(response);
    }
    //@Override
    public void onErrorHandler1(String ERROR) {
        iGetCategoriesFullDataService_V1.getCategoriesFullDataServiceError(ERROR);
    }
    //@Override
    ...
    public void resultProcessingForGetCategoriesFullDataService(JSONObject response) {
        try {
            if(response.has("channels")) {
                ...
                if(response.getJSONArray("channels").length() > 0) {
                    spotLightCategoriesDTO.getSpotLightChannelDTOList().clear();
                }

                for (int i = 0; i < response.getJSONArray("channels").length(); i++) {
                    try {
                        JSONObject channel = response.getJSONArray("channels").getJSONObject(i);
                ...
                iGetCategoriesFullDataService_V1.getCategoriesFullDataServiceError("API call broke, Please report!");
            }

            iGetCategoriesFullDataService_V1.getCategoriesFullDataServiceResponse(spotLightCategoriesDTO);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
